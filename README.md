# Computer Science Study Notes

## 1 Intention & Background

When I decided to learn computer science myself, I met with difficulties at first. Now this note is still under construction, and I hope this can be helpful to other people who
have the same experience with me.

## 2 References

For this part, I'd like to express my sincere gratitude to all the professors and TAs listed below, whose lectures really help me along my learning process. I also refer to
some other websites and books. It took me a long time before I realized the importance of mentioning the references, and giving credit to those who make contributions to the 
dispersion of relative knowledge. If someone's original work is used but not mentioned below, I'm sorry for the misuse and please contact me via email. Thanks!

1. Algorithms I & II (Coursera) by Professor Bob Sedgewick & Kevin Weign.
2. CS61A: Structure and Interpretation of Computer Programs (UC Berkeley) by Professor John DeNero and the rest of CS61A staff.
3. CS61B: Data Structures and Algorithms (UC Berkeley) by Professor Josh Hug and the rest of CS61B staff.
4. CS61C: Great Ideas in Computer Architecture (UC Berkeley) by Instructors Stephan Kaminsky, Sean Farhat, Jenny Song, and the rest of CS61C staff.
5. CS162: Operating System (UC Berkeley) by Professor John Kubiatowicz and the rest of CS162 staff.
6. By Aeroid - Own work, CC BY-SA 4.0, https://commons.wikimedia.org/w/index.php?curid=137790829.
