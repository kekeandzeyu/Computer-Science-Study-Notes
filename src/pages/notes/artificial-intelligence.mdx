...
There are mainly two types of agents: <span style="color:Fuchsia">reflex agents</span> and <span style="color:Fuchsia">planning agents</span>.

1. <span style="color:Fuchsia">Reflex Agents:</span>
...
2. <span style="color:Fuchsia">Planning Agents:</span>
...
* <span style="color:Fuchsia">State Space Graph:</span> A mathematical representation of a search problem. It consists of nodes (abstarcted world configurations) and arcs (successors/action results). The goal test is a set of goal nodes (maybe only one).
* *<span style="color:Fuchsia">Search Trees:</span> A "what if" tree of plans and their outcomes. Nodes show states, and children correspond to successors.

Search algorithms can be divided into two categories:

* <span style="color:Fuchsia">Uninformed Search:</span> No additional information about states beyond the definition of the problem.
    * Depth-First Search
    * Breadth-First Search
    * Uniform-Cost Search
* <span style="color:Fuchsia">Informed Search:</span> Use heuristics to guide the search.
    * A* Search
...
1. <span style="color:Fuchsia">Manhattan Distance:</span> The sum of the absolute differences of the $x$ and $y$ coordinates. 

$$
h\left(n\right) = \left|x_{\text{goal}} - x_{\text{current}}\right| + \left|y_{\text{goal}} - y_{\text{current}}\right|
$$

2. <span style="color:Fuchsia">Euclidean Distance:</span> The straight-line distance between two points.

$$
h\left(n\right) = \sqrt{\left(x_{\text{goal}} - x_{\text{current}}\right)^2 + \left(y_{\text{goal}} - y_{\text{current}}\right)^2}
$$

3. <span style="color:Fuchsia">Chebyshev Distance:</span> The maximum of the absolute differences of the $x$ and $y$ coordinates.

$$
h\left(n\right) = \max\left(\left|x_{\text{goal}} - x_{\text{current}}\right|, \left|y_{\text{goal}} - y_{\text{current}}\right|\right)
$$

4. <span style="color:Fuchsia">Octile Distance:</span> The diagonal distance between two points.
...
5. <span style="color:Fuchsia">Zero Heuristic:</span> The heuristic that always returns zero, which makes it the same as uniform-cost search.

<p style="color:BlueViolet">Properties</p>

1. <span style="color:Fuchsia">Admissible heuristics:</span> A heuristic $h$ is admissible (optimistic) if:
...
2. <span style="color:Fuchsia">Consistency:</span> The estimate of heuristic function is always less than or equal to the estimated distance from any neighbouring vertex to the goal, plus the cost of reaching that neighbour.
...
$$
   ...
   <span style="color:Fuchsia">Optimality:</span>  A* search, with a consistent heuristic, is optimally efficient. This result does not hold if A*'s heuristic is admissible but not consistent.

   For tree search, A* is optimal if heuristic is admissible; for graph search, A* is  optimal if heuristic is consistent.

3. <span style="color:Fuchsia">Completeness:</span> On finite graphs with non-negative edge weights A* is guaranteed to terminate and is complete, i.e. it will always find a solution (a path from start to goal) if one exists.
